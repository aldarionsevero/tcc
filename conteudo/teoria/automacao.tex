...
utiliza ou pretende utilizar a ferramenta Chef. O Chef é uma ferramenta de
automação de configuração de infraestrutura que utiliza o conceito de infraestrutura
como código para orquestração das configurações de infraestrutura. Para exemplificar
a simplificação entre \textit{scripts} "manuais" e os \textit{scripts} em Chef, os códigos
~\ref{code:shell} e~\ref{code:chef} mostram um \textit{script} escrito em
\textit{shell} e outro em \textit{Ruby}, que é interpretada pelo Chef.
